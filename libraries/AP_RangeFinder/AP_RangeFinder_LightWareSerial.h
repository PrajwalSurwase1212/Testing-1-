...
private:
    ...
    uint32_t last_init_ms{0};      // init time used to switch lw20 to serial mode
    uint8_t high_byte{0};          // binary protocol high byte
    bool high_byte_received{false};    // true if high byte has been received
    ...
    uint8_t legacy_valid_count{0};
    uint8_t binary_valid_count{0};
